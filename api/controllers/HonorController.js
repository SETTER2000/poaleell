 ...
 * HonorController
 *
 * @description :: Server-side logic for managing honors
...
module.exports = {
    upload: function (req, res) {
        const pathUpload = 'assets/images/honor/'+req.session.me;
        console.log('sails.config.appPath', sails.config.appPath + pathUpload);
        req.file('file').upload({
            ...
            function (err, files) {
                ...
                const pathToReport = "assets/images/honor/report/" + nameFileUpload;
                ...
                XlsxPopulate.fromFileAsync(files[0].fd)
                    .then(
                        function (workbook) {
                            ...
                            Honor.destroy({vendor: vendor}).exec(function (err, destr) {
                                if (err) sails.log('Ошибка при удалении записей из БД');
                                let price = all.rowsValidArr();
                                if (price.length > 0) {
                                    Honor.findOrCreate(price, price).exec(
    ...
    download: function (req, res, next) {
         ...
         */
        const pathPrice = 'assets/images/honor/full/';

        /**
         * Шаблон для полного прайса
         */
        const templateOutPricePath = "assets/images/honor/ideal/full_out.xlsx";

        Honor.find({}, {updatedAt: 1, _id: 0})
            ...
            if (datePrice.length) {
                ...
                Honor.find().sort({vendor: 1}).exec(function userCreated(err, price) {
                    if (err) return res.negotiate(err);
                    if (!price) return res.notFound();
                    let io = 1;
