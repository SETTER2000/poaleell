            ...
            XlsxPopulate.fromFileAsync(pathToXlsxFile)
                .then(
                    function (workbook) {
                        ...
                        //            // Объект all будет содержать общую информацию по книге
                        const all = new Ranges(workbook, 'ALL', `A1:J${allRows}`);
                        const data = new Ranges(workbook, 'DATA', `A9:J${allRows}`); // данные книги
                        const named = new Ranges(workbook, 'NAMED', 'A1:F1');
                        const info = new Ranges(workbook, 'INFO', 'C3:D5');
                        const header = new Ranges(workbook, 'HEADER', 'A7:E7');
                        const headerTwo = new Ranges(workbook, 'HEADERTWO', 'E8:F8');
                        ...
                        //let currentCell = cell.columnName() + '' + cell.rowNumber();
                        //
                        //// Данные ячейки
                        //let valueCell = `${cell.value()}`;
                        //
                        ////sails.log(currentCell + ': ' + valueCell);
                        //
                        //if (cell.columnName() == 'C' && valueCell != 'undefined') {
                        //    name = valueCell;
                        //    valueCell = valueCell.match(/([а-яё]+)/gi);
                        //}
                        //
                        //if (cell.columnName() == 'E' && valueCell != 'undefined') {
                        //    startPeriod = valueCell;
                        //}
                        //
                        //if (cell.columnName() == 'A' && valueCell != 'undefined') {
                        //    date = valueCell;
                        //}
                        //
                        //
                        ////sails.log('startPeriod');
                        ////sails.log(name + ': ' + startPeriod);
                        //
                        //User.findOne({
                        //        lastName: valueCell[0],
                        //        firstName: valueCell[1],
                        //        patronymicName: valueCell[2]
                        //    })
                        ...
                        //            owner: foundUser.id
                        //        }).exec(function (err, createdSKD) {
                        //            if (err) console.log('error25');
                        //            //sails.log('Сохранил успешно!! Пользователь ID: ' + name + ' Приход: ' + startPeriod);
                        //        });
                        //    });


                        //sails.log(workbook.sheet(0).cell(currentCell).value());
